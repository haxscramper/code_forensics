...
def impl(args):
    ...
    # Analyze latest commit. Later on multiple data frames can be
    # aggregated together based on `groupby("commit_time")`
    ...
    df = (
        # 'count' or 'total' field is not necessary here, so direct
        # aggregate can be used
        df.groupby(["file_dir", "file_name"])
        .agg(
            # count number of lines in each category separately - number of
            # categories is fixed (due to hardcoded detection logic), so
            # column names can be hardcoded here as well
            comment=("category", lambda x: x.loc[x == 1].count()),
            code=("category", lambda x: x.loc[x == 0].count()),
            empty=("category", lambda x: x.loc[x == 2].count()),
            # grouped by file directories, so dropping duplicates
            # and taking first element in series is safe here
    ...
    # If per-directory breakdown was requested - create it, otherwise use a
    # simplified approach
    if args.subdir_breakdown:
        ...
        max_name = (
            df["file_dir"].apply(lambda x: len(x))
            + df["file_name"].apply(lambda x: len(x))
        ).max()
    ...
    ax.barh(
        df["name"],
        df["code"],
        left=df["comment"],  # Offset is based on the 'comment' field used
        # previously
        color="blue",
        edgecolor="black",
        label="Code count",
    ...
    ax.barh(
        df["name"],
        df["empty"],
        left=df["comment"] + df["code"],  # Building on top of the existing
        # values
        color="gray",
        label="Empty",
    )
    ...
    # Verbose title with some numerical statistics for a high-level
